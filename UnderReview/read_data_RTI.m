...
[Y,FS] = audioread('running_outside_20ms.wav'); % Remove NBITS because it's not used
...
%RTI plot
figure(10);
v = db(ifft(sif,zpad,2)); % Use db() instead of dbv()
S = v(:,1:size(v,2)/2);
m = max(max(v));
...
S = db(S(:,1:size(v,2)/2)); % Use db() instead of dbv()
m = max(max(S));
