...
target_range = 2;      % Distance from radar to the fan center (meters)
r_fan = 0.5;           % Radius of the fan blades (meters)
omega = .5;            % Angular velocity of fan (radians/second)
...
RDM = abs(sig_fft2);
RDM = 10 * log10(RDM);
doppler_axis = linspace(-50, 50, Nd);
range_axis = linspace(-50, 50, Nr / 2) * ((Nr / 2) / 400);
